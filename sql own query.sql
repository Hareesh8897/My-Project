...
SELECT
s.state AS State_name, COUNT (cr.parliament_constituency) AS Totsl_Seats
FROM
constituencywise_results cr INNER JOIN statewise_results sr ON cr.Parliament_Constituency = sr.Parliament_Constituency
...
--5) Total Seats Won by I.N.D.I.A. Allinace
...
--- add a table into partywise_results-> like., party_alliance
...
FROM 
constituencywise_results cr INNER JOIN partywise_results pr ON cr.Party_id = pr.party_id INNER JOIN statewise_results sr ON 
cr.Parliament_Constituency = sr.Parliament_Constituency INNER JOIN states s ON sr.state_id = s.state_id WHERE cr.Constituency_Name = 'BELLARY'
...
SELECT
   ...
   cd.Party,
   cd.EVM_Votes,
   cd.Postal_Votes, cd.Total_Votes,
